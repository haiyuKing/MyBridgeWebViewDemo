...
public class MyBridgeWebView extends BridgeWebView {
	...
	private void initWebViewSettings(boolean canZoom) {
...
//		webSetting.setDatabasePath(mContext.getApplicationContext().getDir("databases", 0).getPath());
		...
		myBridgeWebChromeClient = new MyBridgeWebChromeClient(getContext(),this);
		this.setWebChromeClient(myBridgeWebChromeClient);
		//使用WebViewClient的特性处理html页面
		myBridgeWebViewClient = new MyBridgeWebViewClient(getContext(),this);
		this.setWebViewClient(myBridgeWebViewClient);

		//实现html文件中可以调用java方法
		myBridgeWebViewJSInterface = MyBridgeWebViewJSInterface.getInstance(mContext,this);
	...
	public void loadWebUrl(String webUrl){
		//Log.w("MyWebView","{loadWebUrl}webUrl="+webUrl);
		this.loadUrl(webUrl);
	}
	...
	public MyBridgeWebChromeClient getMyBridgeWebChromeClient() {
		return myBridgeWebChromeClient;
	}
